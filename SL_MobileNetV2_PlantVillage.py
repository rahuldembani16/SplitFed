...
def collect_all_samples(root_dir):
    """Collect all samples from the dataset directory."""
    print(f"Collecting samples from: {root_dir}")
    
    # Check if it's the new structure with train/test subdirectories
    train_dir = os.path.join(root_dir, 'train')
    test_dir = os.path.join(root_dir, 'test')
    
    if os.path.exists(train_dir) and os.path.exists(test_dir):
        # New structure: train/test split already exists
        train_samples = collect_samples_from_dir(train_dir)
        test_samples = collect_samples_from_dir(test_dir)
        return train_samples, test_samples, None, False  # False = no auto-split needed
    else:
...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch):
    ...
    if count1 == len_batch:
        ...
        print('Train Round: {:3d}, Average Accuracy: {:.3f}%, Average Loss: {:.4f}'.format(l_epoch_count, acc_avg_train, loss_avg_train))
...
def evaluate_server(fx_client, y, idx, len_batch, ell):
    ...
    global acc_test_collect, loss_test_collect, count2, num_users, acc_avg_train_all, loss_avg_train_all
    ...
    with torch.no_grad():
        ...
        if count2 == len_batch:
            ...
            print('Test Round: {:3d}, Average Accuracy: {:.3f}%, Average Loss: {:.4f}'.format(ell, acc_avg_test, loss_avg_test))
...
class Client(object):
    ...
    def train(self, net_client, net_server):
        ...
        return net_client.state_dict(), net_server.state_dict(), sum(epoch_loss) / len(epoch_loss), sum(epoch_acc) / len(epoch_acc)
    
    def evaluate(self, net_client, net_server, ell):
        net_client.eval()
        net_server.eval()
...
# Setup dataset
try:
    train_samples, val_samples, classes, class_to_idx = setup_dataset('./dataset/plantvillage')
...
for iter in range(epochs):
    ...
    for idx in idxs_users:
        ...
        # Training ------------------
        w_client, w_server, _, _ = local.train(net_client=copy.deepcopy(net_glob_client).to(device), 
                                              net_server=copy.deepcopy(net_glob_server).to(device))
        w_locals_client.append(copy.deepcopy(w_client))
        w_locals_server.append(copy.deepcopy(w_server))
        
        # Testing -------------------
        local.evaluate(net_client=copy.deepcopy(net_glob_client).to(device), 
                      net_server=copy.deepcopy(net_glob_server).to(device), ell=iter)
...
# Save client model
torch.save(net_glob_client.state_dict(), 'model/SL_MobileNetV2_client_PlantVillage_final.pth')
print("✅ Client model saved to model/SL_MobileNetV2_client_PlantVillage_final.pth")

# Save server model
torch.save(net_glob_server.state_dict(), 'model/SL_MobileNetV2_server_PlantVillage_final.pth')
print("✅ Server model saved to model/SL_MobileNetV2_server_PlantVillage_final.pth")

# Save training results
import pandas as pd
results_df = pd.DataFrame({
    'Round': range(1, len(acc_test_collect) + 1),
    'Test_Loss': loss_test_collect,
    'Test_Accuracy': acc_test_collect
})
results_df.to_excel('model/SL_MobileNetV2_PlantVillage_results.xlsx', index=False)
print("Training results saved to model/SL_MobileNetV2_PlantVillage_results.xlsx")